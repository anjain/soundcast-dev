...
jQuery('#reviewapp_form_submit').click(function(){
    ...
    data+= '&shop_id=3927805';
    //if (jQuery.browser.msie === true) {
    if ('XDomainRequest' in window && window.XDomainRequest !== null) {
    	//use XDomainRequest for IE8+
	    var xdr = new XDomainRequest();
	    xdr.open('post', 'http://ainaz.pair.com/~chwan/rev/client/addreview');
	    xdr.onload = function () {
	    ...
	    jQuery.ajax({
	        url: 'http://ainaz.pair.com/~chwan/rev/client/addreview',
	        dataType: 'json',
	        type: 'POST',
	        data: data,
...
jQuery('.review-item-report a').click(function(){
    ...
    if (confirm('Are you certain you want to report this review?')) {
        jQuery.ajax({
            url: 'http://ainaz.pair.com/~chwan/rev/client/reportreview',
            type: 'POST',
            dataType: 'json',
            data: {
                review_id : reviewId
