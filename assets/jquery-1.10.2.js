...
jQuery.extend({
	...
	ready: function( wait ) {
		...
		if ( jQuery.fn.trigger ) {
			$( document ).trigger("ready").off("ready");
		}
	},
	...
	valHooks: {
		...
		select: {
			...
			set: function( elem, value ) {
				...
				while ( i-- ) {
					option = options[ i ];
					if ( (option.selected = jQuery.inArray( $(option).val(), values ) >= 0) ) {
						optionSet = true;
					}
				}
	...
	jQuery.valHooks[ this ] = {
		set: function( elem, value ) {
			if ( jQuery.isArray( value ) ) {
				return ( elem.checked = jQuery.inArray( $(elem).val(), value ) >= 0 );
			}
		}
	};
...
jQuery.fn.extend({
	...
	index: function( elem ) {
		...
		if ( typeof elem === "string" ) {
			return jQuery.inArray( this[0], $( elem ) );
		}
...
function actualDisplay( name, doc ) {
	var elem = $( doc.createElement( name ) ).appendTo( doc.body ),
